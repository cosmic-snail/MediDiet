...
审核等待模式必须只使用与当前患者匹配的 `ReviewCase`。如果找不到匹配项，应保持当前患者推荐为空，而不能回退到其它患者的审核 case；这样可以避免把其它患者的 trace、解释或审核状态写入当前患者。
...
健康资料摘要中的概念编码使用统一显示函数转换。已知编码显示为中文标签；未知编码将下划线替换为空格后直接显示，便于在原型阶段暴露未覆盖的概念值。
...
`seedDemoData` 仍通过现有 `GET /debug/state` 判断后端内存状态。摄入记录采用按当前患者计数的增量补写策略：如果后端已有 `N` 条当前患者摄入记录，前端只提交当前输入中从第 `N + 1` 条开始的记录。该策略适用于当前 append-only 原型；如果后续摄入记录支持编辑、删除或重排，需要改为基于稳定 `intakeId` 的同步策略。
...
- `medidietApi` 只为当前患者补写缺失摄入记录，切换患者 seed 时不会混入上一位患者摄入记录。
- 未知概念编码在患者摘要中可见，避免静默隐藏未映射数据。
- 当前患者没有匹配审核 case 时，不会 fallback 到其它患者的审核 trace。
...
- 获取推荐时只提交当前患者缺失的摄入记录，不重复写入已 seed 记录，也不混入其它患者记录。
